 ...
 * @since		1.0
 */
class ProductfinderModelQuestionnaires extends JModelList
{
 ...
 */
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.title, a.alias, a.description, a.image ' .
	...
	public function getAuthors() {
		...
		$query->join('INNER', '#__pf_questionnaires AS c ON c.created_by = u.id');
	...
	public function getTable($name = 'Questionnaires', $prefix = 'ProductfinderTable', $options = array())
	{
		return parent::getTable($name, $prefix, $options);
	}

	/**
	 * Toggles on or more questionnaire(s) published state
	 * 
	 * @param string $pks
	 * @param number $state
	 * @param number $userId
	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		$table = $this->getTable();
		if(! $table->publish($pks, $state, $userId)){
	...
	public function saveorder($pks = null, $order = null)
	{
		...
		if (empty($pks))
		{
			return JError::raiseWarning(500, JText::_('COM_PRODUCTFINDER_ERR_NO_QUESTIONNAIRES_SELECTED'));
		}
	...
	public function delete(){
		...
		if(!$table->delete($toDel)){
			$this->setError(JText::_('COM_PRODUCTFINDER_ERR_CANT_DELETE_QUESTIONNAIRES'));
			return false;
		}
